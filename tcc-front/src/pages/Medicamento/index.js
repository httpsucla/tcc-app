            ...
            for (let i = 0; i < gavetas.length; i++) {
              if (item.id == gavetas[i].id_medicamento) {
                ...
                Alert.alert(
                  ...
                  'Medicamento ' +
                    item.nome +
                    ' foi removido com sucesso. Gaveta ' +
                    (gavetas[i].id + 1) +
                    ' está vazia agora!'
                )
                break
                ...
                Alert.alert(
                  'Sucesso',
                  'Medicamento ' + item.nome + ' removido com sucesso.'
                )
                break
              }
            }
  ...
  return (
    ...
    >
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate('Cadastrar Medicamento')}
      >
          ...
          renderItem={({ item }) => (
            <View style={styles.campolista}>
              <View style={styles.campoconteudo}>
                <TouchableOpacity
                  onPress={() =>
                    navigation.navigate('Visualizar Medicamento', {
                      item: item
                    })
                  }
                >
